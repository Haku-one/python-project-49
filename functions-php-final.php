<?php
/**
 * ФИНАЛЬНАЯ СДЭК Интеграция для WooCommerce
 * Замените весь предыдущий код этим
 */
...
// Добавляем контейнер для блоков checkout прямо в footer
add_action('wp_footer', function() {
    if (is_checkout()) {
        ?>
        <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Функция для добавления контейнера
            function addCdekContainer() {
                // Находим поле адреса
                const addressField = document.querySelector('.wc-block-components-address-form__address_1');
                if (!addressField) {
                    setTimeout(addCdekContainer, 500);
                    return;
                }
                
                // Проверяем, не добавлен ли уже контейнер
                if (document.getElementById('cdek-blocks-pickup-container')) {
                    return;
                }
                
                // Создаем контейнер
                const cdekDiv = document.createElement('div');
                cdekDiv.id = 'cdek-blocks-pickup-container';
                cdekDiv.className = 'cdek-pickup-container';
                cdekDiv.innerHTML = `
                ...
                addressField.parentNode.insertBefore(cdekDiv, addressField.nextSibling);
                
                console.log('CDEK container added to blocks checkout');
            }
            
            // Запускаем добавление контейнера
            addCdekContainer();
            
            // Повторяем попытки если нужно
            setTimeout(addCdekContainer, 1000);
            setTimeout(addCdekContainer, 2000);
        });
        </script>
        <?php
    }
