<?php
/**
 * СДЭК Интеграция для WooCommerce - ИСПРАВЛЕННАЯ версия
...
class CDEK_WooCommerce_Integration {
    ...
    public function init_shipping_method() {
        if (!class_exists('WC_CDEK_Shipping_Method')) {
            include_once('class-wc-cdek-shipping-method.php');
        }
    }
...
// Класс метода доставки СДЭК - определяем прямо здесь
if (!class_exists('WC_CDEK_Shipping_Method') && class_exists('WC_Shipping_Method')) {
    class WC_CDEK_Shipping_Method extends WC_Shipping_Method {
        ...
        public function __construct($instance_id = 0) {
            ...
            $this->method_description = 'Доставка через службу СДЭК до пунктов выдачи';
            $this->enabled = 'yes';
...
// Создаем зону доставки и добавляем СДЭК при активации
add_action('admin_init', function() {
    // Проверяем, создана ли уже зона доставки
    if (get_option('cdek_shipping_zone_created')) {
        return;
    }
    ...
    try {
        // Создаем зону доставки для России
        $zone = new WC_Shipping_Zone();
        $zone->set_zone_name('Россия (СДЭК)');
        $zone->add_location('RU', 'country');
        $zone->save();
        
        // Добавляем метод доставки СДЭК в зону
        $zone->add_shipping_method('cdek_shipping');
        
        // Отмечаем, что зона создана
        update_option('cdek_shipping_zone_created', true);
        
        // Добавляем уведомление об успешном создании
        add_action('admin_notices', function() {
            echo '<div class="notice notice-success is-dismissible">';
            echo '<p><strong>СДЭК:</strong> Зона доставки "Россия (СДЭК)" создана автоматически. Настройте её в разделе WooCommerce → Настройки → Доставка.</p>';
            echo '</div>';
        });
        
    } catch (Exception $e) {
        error_log('CDEK Zone Creation Error: ' . $e->getMessage());
    }
});

// Добавляем admin notice для настройки
add_action('admin_notices', function() {
    if (!get_option('cdek_shipping_configured')) {
        $screen = get_current_screen();
        if ($screen && strpos($screen->id, 'woocommerce') !== false) {
            ?>
            <div class="notice notice-info">
                <p><strong>СДЭК Интеграция:</strong> Не забудьте настроить зону доставки в <a href="<?php echo admin_url('admin.php?page=wc-settings&tab=shipping'); ?>">WooCommerce → Настройки → Доставка</a></p>
                <p><a href="#" onclick="jQuery(this).closest('.notice').fadeOut(); jQuery.post(ajaxurl, {action: 'cdek_hide_notice'});" class="button-secondary">Скрыть это сообщение</a></p>
            </div>
            <?php
        }
    }
...
// AJAX для скрытия уведомления
add_action('wp_ajax_cdek_hide_notice', function() {
    update_option('cdek_shipping_configured', true);
    wp_die();
});