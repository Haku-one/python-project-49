...
class CDEK_WooCommerce_Integration {
    ...
    public function init_cdek_shipping_method() {
        if (!class_exists('WC_CDEK_Shipping_Method')) {
            // Определяем класс метода доставки отдельно
            $this->define_shipping_method_class();
        }
    }
     ...
     */
    private function define_shipping_method_class() {
...
// Класс для интеграции с блоками checkout (отдельно от основного класса)
...
// Инициализируем интеграцию (обязательно после объявления классов)
 ...
 */
if (!function_exists('cdek_add_pickup_to_checkout_blocks')) {
